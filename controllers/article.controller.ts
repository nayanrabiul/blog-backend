...
export const fetchTopic = async (req, res, next) => {
    try {
        const { query } = req;
        const topic = await Topic.findOne({ _id: query._id });
...
export const fetchTopics = async (req, res, next) => {
    try {
        let data = {};
        data = await Topic.find({}, ['name']);
        return res.status(200).json({
            error: false,
...
export const postTopic = async (req, res) => {
    try {
        ...
        if (!!body._id) {
            await Topic.findOneAndUpdate({ _id: body._id }, body);
...
export const delTopic = async (req, res, next) => {
    try {
        const { query } = req;
        await Topic.findByIdAndDelete(query._id);
        return res.status(200).json({
            error: false,
...
export const fetchArticles = async (req, res, next) => {
    ...
    try {
        ...
        const created_by = new mongoose.Types.ObjectId(user._id);
...
export const paginatedFetchArticles = async (req, res, next) => {
    ...
    try {
        ...
        if (topic_id) {
            matchStage.topic = new mongoose.Types.ObjectId(topic_id);
        }
        ...
        let users = await Article.aggregate([
            {
                $match: matchStage,
            },
            {
...
export const fetchArticle = async (req, res, next) => {
    try {
        ...
        console.log(query);
        if (!!query._id) {
            let data = await Article.findById(query._id)
                .populate('topic')
        ...
        if (!!query.key) {
            let data = await Article.findOne({ key: query.key })
                .populate('topic')
                .populate('user', 'name username email');
            return res.status(200).json({
                error: false,
...
export const fetchFeatureArticle = async (req, res, next) => {
    let topic_id = req.query.topic_id;
    try {
        let data = null;
        if (topic_id) {
            data = await Article.findOne({ topic: topic_id });
        } else {
...
export const fetchAllArticlesLink = async (req, res, next) => {
    try {
...
export const postArticle = async (req, res) => {
    try {
        ...
        if (!!body._id) {
            ...
            await Article.findOneAndUpdate({ _id: body._id }, {
...
export const deleteArticle = async (req, res) => {
    ...
    try {
        ...
        const data = await Article.findByIdAndDelete(_id);
        if (!data) {
            return res.status(400).send({
                error: true,
